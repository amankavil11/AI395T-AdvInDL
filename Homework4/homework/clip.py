...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Get the pooled/cls token output (typically last_hidden_state[:, 0])
        ...
        if hasattr(text_outputs, 'pooler_output') and text_outputs.pooler_output is not None:
            ...
            # Use mean pooling over sequence (better for variable length text)
        ...
        # L2 normalize (add small epsilon to avoid division by zero)
        vision_features = vision_features / (torch.norm(vision_features, dim=-1, keepdim=True) + 1e-8)
        text_features = text_features / (torch.norm(text_features, dim=-1, keepdim=True) + 1e-8)
...
def compute_clip_loss(
    ...
    # Use logits from forward pass (already includes temperature scaling)
    # logits is vision_features @ text_features.T / temperature
    logits_per_image = logits  # Shape: [batch_size, batch_size]
    logits_per_text = logits.T  # Shape: [batch_size, batch_size]
